function [CM, V_CM, V_CM_smooth, V_rms_CM, V_rms_CM_all, V_rms_lab, V_rms_lab_CORR, V_fake] = GetVelocities_C_R(n_frames,tab_spots,SmoothingWindow,V_CM_all)    % dist_edge,img_size)
...
V_rms_CM_all = zeros(n_frames-1,1);
V_rms_lab = zeros(n_frames-1,1);
V_rms_lab_CORR = zeros(n_frames-1,1);
x_CM = zeros(n_frames,1);
y_CM = zeros(n_frames,1);

% CM velocity components whole cell monolayer
V_CM_x_all = V_CM_all(:,1);
V_CM_y_all = V_CM_all(:,2);
...
for jjj = 1 : n_frames    % time
    ...
    x_i = tab_spots.POSITION_X(idx_final);
    y_i = tab_spots.POSITION_Y(idx_final);

    % Instantaneous velocity components (lab sdr)
    v_x_i = tab_spots.dX(idx_final);
    v_y_i = tab_spots.dY(idx_final);
...
%     y_i_cm = y_i - y_CM(jjj);
    ...
    % V_rms (CM sdr)
    V_rms_CM(jjj) =  mean( ((v_x_i - V_CM_x(jjj)).^2 + (v_y_i - V_CM_y(jjj)).^2), 'omitnan') ;

    % V_rms (CM whole monolayer sdr)
    V_rms_CM_all(jjj) =  mean( ((v_x_i - V_CM_x_all(jjj)).^2 + (v_y_i - V_CM_y_all(jjj)).^2), 'omitnan') ;
    
    % V_rms (lab sdr)
    V_rms_lab(jjj) =  mean( (v_x_i.^2 + v_y_i.^2), 'omitnan') ;
...
for jjj = 1 : n_frames   % time
    ...
    v_x_i = tab_spots.dX(idx_final) - V_fake_x(jjj);
    v_y_i = tab_spots.dY(idx_final) - V_fake_y(jjj);
    
    % V_rms (lab sdr) - CORRECTED
    V_rms_lab_CORR(jjj) = sqrt( mean( ((v_x_i).^2 + (v_y_i).^2), 'omitnan') );
