def mixedsegmentation(filename, params):
    ...
    global nuclei_raw, red_nuclei_raw, green_nuclei_raw
    ...
    if (image_shp==3):    
        temp=(image[cell_sig],image[prot_sig],image[red_nu],image[green_nu])
        img=np.asarray(temp)
    if (image_shp==4):
        ...
        # Extract channels
    cell_raw=img[0].copy()
    protein_raw=img[1].copy()
    red_nuclei_raw=img[2].copy()
    green_nuclei_raw=img[3].copy()
    nuclei_raw=red_nuclei_raw+green_nuclei_raw
    ...
    print("Done!", flush=True)
    ################### NUCLEI SEGMENTATION ##############################
    nuclear_type = ['', 'red_', 'green_']
    masks_nu_fil = []
    
    for h, var_name in enumerate(nuclear_type):
        varname = f'{var_name}nuclei_raw'
        nuclei_loop = globals()[varname]
        # Gamma correction (x^gamma)
        nuclei_gamma=np.power(nuclei_loop,params.get('gamma_nu'))
        # Gaussian blur
        nuclei_blur=filters.gaussian(nuclei_gamma, sigma=params.get('gaussian_nu'))
        
        print(f'Segmentation of {var_name}Nuclei with Cellpose...', flush=True)
        # Define channels
        ch_nu=[0,0]
        ...
        props_nu_fil_2 = measure.regionprops(masks_nu_fil_2,nuclei_loop)
        ...
        # Final mask
        masks_nu_filt=masks_nu_fil_3.copy()
        masks_nu_fil.append(masks_nu_filt)
        print("Done!", flush=True)
    
        io.save_masks(nuclei_blur, 
                  masks_nu_filt, 
                  flows_nu, 
                  file_names = os.path.splitext(filename)[0]+'_'+nuclear_type[h]+'nuclei',
                  channels=ch_nu,
                  png=True, # save masks as PNGs and save example image
                  tif=False, # save masks as TIFFs
                  save_txt=False, # save txt outlines for ImageJ
    ...
    for x, var_name in enumerate(nuclear_type):
        varname = f'{var_name}nuclei_raw'
        nuclei_loop = globals()[varname]
        print(f'Filtering cells and {var_name}nuclei...', flush=True)
        # Filtering of nuclei
        props_nu_fil = measure.regionprops(masks_nu_fil[x],nuclei_loop)
        masks_nu_cell_fil_1 = masks_nu_fil[x].copy() 
        
        for i in range(0,len(props_nu_fil)):
            print(f'{i} of {len(props_nu_fil)}')
            masks_single_nu_in_cell=np.where(masks_nu_fil[x]==props_nu_fil[i].label,masks_cell_fil,0)
            props_masks_single_nu_in_cell=measure.regionprops(masks_single_nu_in_cell,cell_raw)
            
            if (len(props_masks_single_nu_in_cell)>2):
                masks_nu_cell_fil_1[masks_nu_fil[x]==props_nu_fil[i].label]=0
        ...
        # Filtering of cells      
        masks_cell_fil_4 = masks_cell_fil.copy()              
        props_cell = measure.regionprops(masks_cell_fil_4,cell_raw)

        for i in range(len(props_cell)):
            masks_single_cell=np.where(masks_cell_fil_4==props_cell[i].label,masks_cell_fil_4,0)
            masks_nu_in_single_cell=np.where(masks_single_cell!=0,masks_nu_cell_fil_1,0)
            props_nu_in_single_cell=measure.regionprops(masks_nu_in_single_cell,nuclei_loop)
            if (len(props_nu_in_single_cell)==0):
                masks_cell_fil_4[masks_cell_fil_4==props_cell[i].label]=0
                    
        # Final masks generation
        masks_cell_fil_final=masks_cell_fil_4.copy()
        masks_nu_cell_fil=masks_nu_cell_fil_1.copy()
        
        # Create masks of nuclei with the same index as cells
        masks_nu_in_cell=np.where(masks_nu_cell_fil!=0,masks_cell_fil_final,0)
        
        print("Done!", flush=True)
        
        # Create masks of cytoplasm where there is no nucleus
        masks_cyto_only=np.where(masks_nu_cell_fil==0,masks_cell_fil_final,0)
        ...
        props_protein_in_cell = measure.regionprops(masks_cell_fil_final,protein_raw)
        ...
        plt.imshow(nuclei_loop+cell_raw, vmin=np.percentile(nuclei_loop,min(vmin_nu,vmin_cell)), vmax=np.percentile(nuclei_loop,max(vmax_nu,vmax_cell)), cmap='gray')
        # Plot Nuclear segmentation masks
        plt.imshow(mycmap(masks_nu_in_cell), alpha=0.5)
        # Do not plot axis
        plt.axis("off") 
        # Set title
        plt.title(f'{var_name}Nuclear Masks', fontsize=20, color='white')
        # Save figure
        plt.savefig(output_dir+f'/{var_name}nuclear_segmentation.tif',transparent=True)
        # Close figure
        plt.close()
        print(f'1. Exported {var_name}nuclear masks on cell + nuclear signal!', flush=True)
        
        # 2. Cell Segmentation
        io.save_masks(cell_gamma, 
              masks_cell_fil_final, 
              flows_cell, 
              file_names = os.path.join(finalmasks_dir, os.path.basename(os.path.splitext(filename)[0])+f'_{var_name}final_cell_mask'),
              channels=ch,
              png=True, # save masks as PNGs and save example image
              tif=False, # save masks as TIFFs
              save_txt=False, # save txt outlines for ImageJ
        ...
        plt.imshow(cell_raw+nuclei_loop, vmin=np.percentile(cell_raw,min(vmin_nu,vmin_cell)), vmax=np.percentile(cell_raw,max(vmax_nu,vmax_cell)), cmap='gray')
        # Plot cytoplasm segmentation masks
        plt.imshow(mycmap(masks_cell_fil_final), alpha=0.5)
        # Do not plot axis
        plt.axis("off")
        # Set title
        plt.title(f'{var_name}Cell Masks', fontsize=20, color='white')
        # Save figure
        plt.savefig(output_dir+f'/{var_name}cell_segmentation.tif',transparent=True)  
        # Close Fig
        plt.close()
        print(f'2. Exported {var_name}cell masks on cell + nuclear signal!')
        ...
        plt.axis("off")
        # Set title
        plt.title(f'{var_name}Cyplasmic Masks on Protein Signal', fontsize=20, color='white')
        # Save figure
        plt.savefig(output_dir+f'/{var_name}cyto_mask_on_protein.tif',transparent=True)
        # Close Fig
        plt.close()
        print(f'3. Exported {var_name}cytoplasmic mask on protein signal!', flush=True)
        
        # 4. Labelled cell masks
        # Create figure object
        plt.figure(figsize=(6,6))
        # Plot cell masks
        plt.imshow(mycmap(masks_cell_fil_final))
        ...
        # Set title
        plt.title(f'Labelled {var_name}Cell Masks', fontsize=20, color='white')
        # Do not plot figure axis
        plt.axis('off')
        # Save figure
        plt.savefig(output_dir+f'/{var_name}cell_segmentation_label.tif',transparent=True)
        # Close Fig
        plt.close()
        print(f'4. Exported labelled {var_name}cell masks!', flush=True)
        ...
        # Set title
        plt.title(f'Labelled {var_name}Nuclear Masks', fontsize=20, color='white')
        # Do not plot figure axis
        plt.axis('off')
        # Save figure
        plt.savefig(output_dir+f'/{var_name}nuclear_segmentation_label.tif',transparent=True)
        # Close Fig
        plt.close()
        print(f'5. Exported labelled {var_name}nuclear masks!', flush=True)
        ...
        with open(output_dir+f'/{var_name}properties.csv', 'w') as f:
        ...
        # Close file
        f.close();
        print(f'6. Exported {var_name}table of properties!', flush=True)
